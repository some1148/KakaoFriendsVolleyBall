...
class Mygame extends JPanel implements LineListener { // 선그리기
	...
	int speed_x = 0;
	int speed_y = 5;
	Image back1 = Toolkit.getDefaultToolkit().getImage("gameBackground.png");
	Image back2 = Toolkit.getDefaultToolkit().getImage("snow.gif");
	Image back4 = Toolkit.getDefaultToolkit().getImage("네트.png");

	Image ball = Toolkit.getDefaultToolkit().getImage("ball.png");

	Image player1 = Toolkit.getDefaultToolkit().getImage("player1.gif");
	Image player1_jump = Toolkit.getDefaultToolkit().getImage("player1.gif");

	Image player2 = Toolkit.getDefaultToolkit().getImage("player2.gif");
	Image player2_jump = Toolkit.getDefaultToolkit().getImage("player2.gif");
	
	//슬라이드 기능 잠시 미사용
	//Image player_leftslide = Toolkit.getDefaultToolkit().getImage("leftSlider.png");
	...
	int p1_y = 513; // 1P 캐릭터 그림그리는 y좌표WWW

	int p2_x = 849; // 2P 캐릭터 그림그리는 x좌표
	int p2_y = 513; // 2P 캐릭터 그림그리는 y좌표
	...
	public void getScore(int player) {
		...
		p1_y = 513; // 1P 캐릭터 그림그리는 y좌표

		p2_x = 849; // 2P 캐릭터 그림그리는 x좌표
		p2_y = 513; // 2P 캐릭터 그림그리는 y좌표
	...
	public void Back(Graphics g) {
		...
		g.drawImage(back2, 0, 0, 933, 700, this); // 구름
	...
	public void P1(Graphics g) {
		if (p1_motion == 1) {
			g.drawImage(player1_jump, p1_x, p1_y, 100, 157, this);
		} else if (p1_motion == 2) { //슬라이드 기능 잠시 미사용
			//g.drawImage(player_leftslide, p1_x, p1_y, 100, 157, this);
		} else if (p1_motion == 3) { //슬라이드 기능 잠시 미사용
			//g.drawImage(player_rightslide, p1_x, p1_y, 100, 157, this);
		} else {
			g.drawImage(player1, p1_x, p1_y, 100, 157, this);
		}
	}
	...
	public void P2(Graphics g) {
		...
		if (p2_motion == 1) {
			g.drawImage(player2_jump, p2_x, p2_y, 100, 157, this);
		} else if (p2_motion == 2) { //슬라이드 기능 잠시 미사용
			//g.drawImage(player_leftslide, p2_x, p2_y, 100, 157, this);
		} else if (p2_motion == 3) { //슬라이드 기능 잠시 미사용
			//g.drawImage(player_rightslide, p2_x, p2_y, 100, 157, this);
		} else {
			g.drawImage(player2, p2_x, p2_y, 100, 157, this);
		}
	}
